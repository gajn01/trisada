                ...
                <div class="modal-body">
                    <form wire:submit.prevent="save">
                        ...
                        <div class="settings-form">
                            ...
                            @if (!$is_logistic)
                                <div class="mb-2">
                                    <label for="department_id" class="form-label small">Department</label>
                                    <select id="department_id" name="department_id" wire:model.defer="drivers.department_id" class="form-select form-select-sm" required>
                                        <option value selected hidden>--Select Department--</option>
                                        @foreach ($department_list as $item)
                                            <option value="{{$item->id}}">{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('drivers.department_id')<span class="text-danger mt-1">{{ $message }}</span>@enderror
                                </div>
                            @endif
                            <div class="mb-2">
